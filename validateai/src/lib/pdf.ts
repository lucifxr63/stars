...
export async function generateValidationPDF(data: PDFData, theme: PDFTheme = 'clean'): Promise<void> {
  ...
  doc.text('Confidencial Â· Generado por ValidateAI Â· validus.scouttech.lat', PAGE_W / 2 - 40, 202);
  ...
  if (data.governance_assessment) {
    ...
    if (gov.cap_table_warnings.length > 0) {
      ...
      for (const w of gov.cap_table_warnings.slice(0, 3)) {
        checkPage(8);
        doc.setFontSize(8); doc.setFont('helvetica', 'normal'); doc.setTextColor(...T.bodyMid);
        const wLines = doc.splitTextToSize(`âš   ${w}`, CON_W);
  ...
  if (data.due_diligence) {
    ...
    const stampText = 'Audited by ValidateAI Pro Â· validus.scouttech.lat';
  ...
  for (let i = 1; i <= pageCount; i++) {
    ...
    if (theme === 'dark') {
      ...
      doc.text('ValidateAI Â· valida tu idea antes de construirla', MARGIN, 286);
      ...
      doc.text('ValidateAI Â· valida tu idea antes de construirla', MARGIN, 285);
