...
// Detecta si una fuente entrega datos demo/simulados (en vez de evidencia real).
// El backend marca estos casos con status 'mock'/'demo' o un source que lo indica.
function isDemoSource(d: { status?: string; source?: string } | null | undefined): boolean {
  if (!d) return false;
  const s = (d.status ?? '').toLowerCase();
  const src = (d.source ?? '').toLowerCase();
  return s === 'mock' || s === 'demo' || src.includes('mock') || src.includes('demo') || src.includes('simulad');
}

// Badge de transparencia: deja explícito que el contenido NO es evidencia real de mercado.
function DemoBadge() {
  return (
    <span
      title="Datos demo — no representa evidencia real de mercado"
      className="ml-auto text-[11px] font-semibold text-amber-600 dark:text-amber-400 flex items-center gap-1"
    >
...
// Badge "Conectado" (fuente real) o "Datos demo" (simulada), según corresponda.
    ...
    <div className="space-y-6">
      ...
      <div>
        <div className="flex items-center gap-2 mb-3">
          ...
          {showRedditData && <SourceBadge demo={redditIsDemo} />}
        </div>
      ...
      <div>
        <div className="flex items-center gap-2 mb-3">
          ...
          {showTrendsData && <SourceBadge demo={trendsIsDemo} />}
        </div>
      ...
      {(redditIsDemo || trendsIsDemo) && (
        <p className="text-[11px] text-amber-600/90 dark:text-amber-400/80 leading-relaxed border-t border-dashed border-amber-200/60 dark:border-amber-800/30 pt-3">
          Algunas señales mostradas son <strong>datos demo / simulados</strong> y no representan evidencia
          real de mercado. No tomes decisiones basándote únicamente en ellas.
        </p>
      )}
