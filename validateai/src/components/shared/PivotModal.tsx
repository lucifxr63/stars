    ...
    <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-black/30 backdrop-blur-sm">
      <div className="bg-white rounded-3xl p-6 w-full max-w-md shadow-2xl">
        ...
        <div className="flex items-center gap-3 mb-5">
          ...
          <div>
            <h2 className="font-bold text-gray-900 text-sm">Pivotar idea</h2>
          ...
          <button
            type="button"
            onClick={onClose}
            className="ml-auto p-1.5 rounded-lg hover:bg-gray-100 transition text-gray-400"
          >
        ...
        <div className="bg-gray-50 rounded-xl px-4 py-3 mb-5 border border-gray-100">
          <p className="text-xs font-semibold text-gray-600 mb-1.5">Se copiará de la versión anterior:</p>
          <div className="flex flex-wrap gap-1.5">
            {['Nombre de idea', 'Industria', 'Segmento', 'MVP', 'Respuestas previas'].map((item) => (
              <span key={item} className="text-[10px] font-semibold px-2 py-0.5 rounded-full bg-white border border-gray-200 text-gray-500">
                {item}
              </span>
            ))}
          </div>
        ...
        <div className="flex gap-3">
          <button
            type="button"
            onClick={onClose}
            className="flex-1 py-2.5 border-2 border-gray-200 rounded-xl text-sm font-semibold text-gray-500 hover:border-gray-300 transition"
          >
            Cancelar
          </button>
          <button
