...
const ANIMUS_NAV_ITEMS = [
  { label: 'Dashboard RaaS', path: '/developers?tab=overview', tab: 'overview', Icon: Layers },
  { label: 'Costos Animus', path: '/developers?tab=costs', tab: 'costs', Icon: Zap },
...
function SidebarLogo() {
  const isAnimus = isAnimusDomain() || window.location.pathname.startsWith('/developers') || window.location.pathname.startsWith('/animus');
  return (
    <Link to={isAnimus ? '/developers' : '/dashboard'} className="flex items-center gap-2.5 group">
...
export function Sidebar({ onClose }: SidebarProps) {
  ...
  const isAnimusMode = isAnimusDomain() || location.pathname.startsWith('/developers') || location.pathname.startsWith('/animus');
      ...
      <nav className="flex-1 px-3 py-4 space-y-0.5 overflow-y-auto">
        {isAnimusMode ? (
          <>
            ...
            {ANIMUS_NAV_ITEMS.map(({ label, path, tab, Icon }) => {
        ...
        {isAdmin && !isAnimusMode && (
          <>
            ...
            >
              ...
              <span className="flex-1">Animus Portal</span>
            </Link>
          </>
        )}
      </nav>
