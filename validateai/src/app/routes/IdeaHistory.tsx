...
function CompareView({ versions }: { versions: ValidationVersion[] }) {
  ...
  const VersionSelect = ({ value, onChange }: { value: string; onChange: (id: string) => void }) => (
    <select
      ...
      className="text-sm font-semibold text-gray-700 bg-gray-50 border border-gray-200 rounded-xl px-3 py-2 focus:outline-none focus:ring-2 focus:ring-teal-400"
    >
      {versions.map((v) => (
  ...
  const VersionCard = ({ v, label }: { v: ValidationVersion | undefined; label: string }) => {
    if (!v) return <div className="flex-1 bg-gray-50 rounded-2xl p-6 text-center text-gray-400 text-sm">Selecciona una versión</div>;
      ...
      <div className="flex-1 bg-white border border-gray-100 rounded-2xl p-5">
        <div className="flex items-center gap-3 mb-4">
          ...
          <div>
            ...
            <p className="font-bold text-gray-900 text-sm">{v.idea_name ?? 'Sin nombre'}</p>
        ...
        {v.pivot_reason && (
          <div className="bg-amber-50 rounded-xl p-3 mb-3 border border-amber-100">
            ...
            <p className="text-xs text-gray-700">{v.pivot_reason}</p>
          </div>
        )}
        <Link
          to={`/results/${v.id}`}
    ...
    <div className="space-y-5">
      ...
      {scoreDelta !== null && (
        <div className={`rounded-xl p-3 text-center border ${
          scoreDelta > 0 ? 'bg-green-50 border-green-100' : scoreDelta < 0 ? 'bg-red-50 border-red-100' : 'bg-gray-50 border-gray-100'
        }`}>
          <p className={`text-2xl font-black ${scoreDelta > 0 ? 'text-green-600' : scoreDelta < 0 ? 'text-red-600' : 'text-gray-500'}`}>
...
export function IdeaHistory() {
  ...
  if (!versions || versions.length === 0) {
    return (
      <div className="min-h-screen flex flex-col items-center justify-center bg-gray-50 gap-4">
        <p className="text-gray-500">No se encontró el historial para esta idea.</p>
        <button
  ...
  return (
    <div className="min-h-screen bg-gray-50 text-gray-900 pb-20">
      {/* Header flotante */}
      <header className="sticky top-0 z-30 bg-white/80 backdrop-blur-md border-b-2 border-gray-100 flex items-center h-16 px-4 sm:px-6">
        <div className="w-full max-w-4xl mx-auto flex items-center justify-between">
          <div className="flex items-center gap-4">
            <button
              onClick={() => navigate(`/results/${id}`)}
              className="w-10 h-10 rounded-full border-2 border-gray-100 flex items-center justify-center text-gray-400 hover:text-gray-800 hover:border-gray-200 transition"
      ...
      <main className="w-full max-w-4xl mx-auto px-4 sm:px-6 mt-8">
        <div className="bg-white border-2 border-gray-100 rounded-3xl p-6 sm:p-10 shadow-sm relative overflow-hidden">
            ...
            {viewMode === 'timeline' ? (
              <>
                <div className="mb-8 text-center">
                  ...
                  <h2 className="text-2xl font-black text-gray-900 mb-2">Evolución de la Idea</h2>
              ...
              <>
                <div className="mb-6 text-center">
                  <h2 className="text-2xl font-black text-gray-900 mb-2">Comparar versiones</h2>
                  <p className="text-sm text-gray-500">Compara el score y contexto de dos versiones de tu idea.</p>
                </div>
