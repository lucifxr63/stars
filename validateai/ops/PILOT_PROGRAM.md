...
- **Estados de piloto en sistema:** **Fase 1 + Fase 2 (2026-07) implementadas.**
  ...
  - *Fase 3B:* **multi-admin** — `public.is_admin()` ahora valida contra la tabla `public.admin_users` (`is_active`) con fallback legacy temporal por email. Los operadores se agregan/desactivan por SQL (ver [docs/ADMIN_OPERATORS.md](../docs/ADMIN_OPERATORS.md)); el gate del frontend usa la RPC `get_my_admin_role()`. No rompe las policies existentes que dependen de `is_admin()`.
  - **Pendiente:** UI de gestión de operadores (Fase 3C); retirar el fallback legacy; reactivación de cobro para medir conversión. No hay pilotos activos: la tabla registra solicitudes, no acuerdos.
