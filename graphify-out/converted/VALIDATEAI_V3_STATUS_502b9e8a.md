<!-- converted from VALIDATEAI_V3_STATUS.docx -->
...
Asegurar que las variables de entorno de Supabase incluyan las claves productivas actualizadas (FINTOC_SECRET_KEY, PJUD_WEBHOOK_SECRET, OPENAI_API_KEY, etc.).
Confirmar que la última migración de base de datos SQL (kyc_status y rut) haya sido ejecutada correctamente en el dashboard web de Supabase de producción, debido a problemas de sincronización en el historial CLI.
Asegurar que la configuración CORS de las Edge Functions (ALLOWED_ORIGINS) incluya los dominios finales de producción.