<!-- converted from STARTUPS_ANALYSIS_P1_VALIDATEAI.docx -->
...
Flujo y Fin:
Toma la idea y descripción del usuario y hace un POST al INE para predecir el código CAENES (Clasificador de Actividades Económicas) asociado.
Con el país y CAENES, consulta al API del BCCh (GetSeries) obteniendo series históricas de los últimos 5 años (e.g. IPC General).
Fin: Todos estos datos duros y estadísticos se inyectan en un prompt gigantesco que se envía a gpt-4o-mini. Esto evita que la IA alucine y garantice que el análisis de tamaño de mercado (TAM/SAM) y las métricas sectoriales estén ancladas a datos reales de la economía chilena actual.
...
Flujo y Fin:
Descarga indicadores financieros críticos: Unidades de Fomento (uf_mes, uf_anio) y corr_monetaria anual.
Almacena o actualiza estos datos crudos en formato JSON dentro de la tabla economic_knowledge de Supabase.
Fin: Sirve como base de datos RAG (Retrieval-Augmented Generation). Cuando la función ai-validate genera las "Proyecciones Financieras" o calcula los costos operativos locales, consulta esta tabla y se asegura de hacer los cálculos financieros con el valor exacto de la UF o el IPC al día de la consulta.
